    ...
    <div style={{ backgroundColor: "#FFFFFF" }}>
      ...
      <div style={{ borderBottom: "1px solid #E4E8E5", backgroundColor: "#F2F8F4" }} className="breadcrumb-pad">
        <div style={{ maxWidth: 1280, margin: "0 auto", display: "flex", alignItems: "center", gap: 8 }}>
          ...
          <span style={{ color: "#E4E8E5" }}>/</span>
          <span style={{ fontSize: 12, color: "#3F4B43", fontFamily: "var(--font-jetbrains), monospace" }}>Beef Organ Supplements</span>
        </div>
      </div>
      ...
      <div style={{ maxWidth: 1280, margin: "0 auto" }} className="pad-section px-page">
        ...
        <div style={{ marginBottom: 56, maxWidth: 760 }}>
          <div className="hidden sm:flex" style={{ alignItems: "center", gap: 12, marginBottom: 16 }}>
            ...
            <span style={{ fontFamily: "var(--font-jetbrains), monospace", fontSize: 9, letterSpacing: "0.2em", textTransform: "uppercase", color: "#0F7A5A", whiteSpace: "nowrap" }}>9 Brands Tested</span>
          </div>
          ...
          <p style={{ fontSize: 13, color: "#3F4B43", lineHeight: 1.7, marginBottom: 20 }}>
              ...
              <thead>
                <tr style={{ borderBottom: "1px solid #E4E8E5", backgroundColor: "#F2F8F4" }}>
                  {["Brand", "Organs", "Sourcing", "Certifications", "Price/Serving", "Score", ""].map((h) => (
                    <th key={h} style={{ padding: "12px 16px", textAlign: "left", fontSize: 10, letterSpacing: "0.15em", textTransform: "uppercase", color: "#6B7770", fontFamily: "var(--font-jetbrains), monospace", fontWeight: 500, whiteSpace: "nowrap" }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                  ...
                  <tr key={pick.slug} style={{ borderBottom: "1px solid #F2F8F4", backgroundColor: i % 2 === 0 ? "#F6F8F6" : "#FFFFFF" }}>
                    <td style={{ padding: "12px 16px", verticalAlign: "top" }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                        <div style={{ width: 44, height: 54, backgroundColor: "#F2F8F4", border: "1px solid #E4E8E5", borderRadius: 6, overflow: "hidden", flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center" }}>
                        ...
                        <div>
                          ...
                          {pick.badge && (
                            <span style={{ fontSize: 10, padding: "2px 8px", backgroundColor: pick.badgeColor, color: "#fff", borderRadius: 99, fontFamily: "var(--font-jetbrains), monospace", letterSpacing: "0.05em" }}>{pick.badge}</span>
                          )}
                        </div>
                      </div>
                    </td>
                    <td style={{ padding: "12px 16px", fontSize: 12, color: "#3F4B43", verticalAlign: "top", maxWidth: 160 }}>{pick.organs}</td>
                    <td style={{ padding: "12px 16px", fontSize: 12, color: "#3F4B43", verticalAlign: "top", whiteSpace: "nowrap" }}>{pick.sourcing}</td>
                    <td style={{ padding: "12px 16px", fontSize: 12, color: "#3F4B43", verticalAlign: "top" }}>{pick.certifications}</td>
        ...
        <section style={{ marginBottom: 64 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
            {topPicks.map((pick) => (
              <div key={pick.slug} style={{ border: "1px solid #E4E8E5", borderRadius: 12, overflow: "hidden", backgroundColor: "#F6F8F6" }}>
                {/* Pick header */}
                <div style={{ padding: "20px 24px", borderBottom: "1px solid #E4E8E5", display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 16, flexWrap: "wrap" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "center", width: 44, height: 44, backgroundColor: pick.rank === 1 ? "#1A1714" : "#F2F8F4", borderRadius: 10, flexShrink: 0 }}>
                      <span style={{ fontFamily: "var(--font-jetbrains), monospace", fontSize: 14, fontWeight: 700, color: pick.rank === 1 ? "#FFFFFF" : "#3F4B43" }}>#{pick.rank}</span>
                    </div>
                    <div>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 8 }}>
                    <div style={{ width: 72, height: 88, backgroundColor: "#F2F8F4", border: "1px solid #E4E8E5", borderRadius: 10, overflow: "hidden", display: "flex", alignItems: "center", justifyContent: "center" }}>
                      <img src={pick.image} alt={pick.name} style={{ width: "100%", height: "100%", objectFit: "contain" }} />
                    </div>
                    <ReviewScoreBadge rating={pick.rating} size="sm" />
                    {pick.badge && (
                      <span style={{ fontSize: 10, padding: "3px 10px", backgroundColor: pick.badgeColor, color: "#fff", borderRadius: 99, fontFamily: "var(--font-jetbrains), monospace" }}>{pick.badge}</span>
                    )}
                  </div>
                </div>
                ...
                <div style={{ padding: "20px 24px", display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(240px, 1fr))", gap: 20 }}>
                  <div>
                    <p style={{ fontFamily: "var(--font-jetbrains), monospace", fontSize: 9, letterSpacing: "0.15em", textTransform: "uppercase", color: "#586259", marginBottom: 8 }}>Verdict</p>
                    <p style={{ fontSize: 14, color: "#1A1714", lineHeight: 1.65 }}>{pick.verdict}</p>
                    <div style={{ marginTop: 12, display: "flex", gap: 12, flexWrap: "wrap" }}>
                      <span style={{ fontSize: 11, color: "#3F4B43", fontFamily: "var(--font-jetbrains), monospace" }}>
                        ORGANS: {pick.organs}
                      </span>
                    </div>
                    <div style={{ marginTop: 12 }}>
                      <p style={{ fontSize: 11, color: "#3F4B43", fontFamily: "var(--font-jetbrains), monospace", marginBottom: 4 }}>BEST FOR: {pick.bestFor}</p>
                    </div>
                  </div>
                  <div>
            ...
            {[
              ...
              <div key={step.num} style={{ padding: 24, backgroundColor: "#F6F8F6", borderRight: "1px solid #F2F8F4", borderBottom: "1px solid #F2F8F4" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 10 }}>
                  <p style={{ fontFamily: "var(--font-jetbrains), monospace", fontSize: 10, letterSpacing: "0.2em", color: "#0F7A5A" }}>{step.num}</p>
                  <span style={{ fontFamily: "var(--font-jetbrains), monospace", fontSize: 10, color: "#586259", backgroundColor: "#F2F8F4", padding: "2px 8px", borderRadius: 99 }}>{step.weight}</span>
                </div>
        ...
        <section style={{ marginBottom: 64 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(260px, 1fr))", gap: 16 }}>
            {[
              ...
              <div key={item.organ} style={{ padding: 20, border: "1px solid #E4E8E5", borderRadius: 12, backgroundColor: "#F6F8F6" }}>
                ...
                <p style={{ fontFamily: "var(--font-jetbrains), monospace", fontSize: 10, color: "#6B7770", letterSpacing: "0.08em", textTransform: "uppercase", marginBottom: 8 }}>Key nutrients: {item.nutrients}</p>
                <p style={{ fontSize: 13, color: "#3F4B43", lineHeight: 1.6, marginBottom: 10 }}>{item.why}</p>
                <Link href={item.link} style={{ fontSize: 12, color: "#0F7A5A", fontWeight: 600, textDecoration: "none" }}>Ingredient profile →</Link>
              </div>
            ))}
          </div>
        </section>
        ...
        <section style={{ marginBottom: 64 }}>
          ...
          <div style={{ padding: 28, border: "1px solid #9B2020", borderRadius: 12, backgroundColor: "#FEF9F7" }}>
            <p style={{ fontFamily: "var(--font-jetbrains), monospace", fontSize: 9, letterSpacing: "0.2em", textTransform: "uppercase", color: "#9B2020", marginBottom: 16 }}>IMPORTANT SAFETY NOTICE</p>
        ...
        <section style={{ marginBottom: 64 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 0, border: "1px solid #E4E8E5", borderRadius: 12, overflow: "hidden" }}>
            {faqItems.map((faq, i) => (
              <div key={i} style={{ padding: "22px 24px", borderBottom: i < faqItems.length - 1 ? "1px solid #F2F8F4" : "none", backgroundColor: i % 2 === 0 ? "#F6F8F6" : "#FFFFFF" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(260px, 1fr))", gap: 12 }}>
            {[
              ...
              <Link key={item.href} href={item.href} style={{ textDecoration: "none" }}>
                <div style={{ padding: 20, border: "1px solid #E4E8E5", borderRadius: 12, backgroundColor: "#F6F8F6", height: "100%" }}>
                  <span style={{ fontSize: 10, padding: "2px 8px", backgroundColor: "#F2F8F4", color: "#3F4B43", borderRadius: 99, fontFamily: "var(--font-jetbrains), monospace", marginBottom: 10, display: "inline-block" }}>{item.label}</span>
                  ...
                  <p style={{ fontSize: 13, color: "#3F4B43", lineHeight: 1.6 }}>{item.desc}</p>
                </div>
              </Link>
            ))}
          </div>
