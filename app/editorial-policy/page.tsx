...
const scoring = [
  { range: "9–10", label: "Exceptional", color: "#2D6A4F", desc: "Category-leading formula, full label transparency, third-party verified. Recommended without reservation." },
  { range: "8", label: "Excellent", color: "#3A7D44", desc: "Strong formula with minor gaps. Worth buying for most users in this category." },
  { range: "7", label: "Good", color: "#6B8F4A", desc: "Solid product with meaningful limitations. May suit some users better than others — see the review for context." },
  { range: "5–6", label: "Average", color: "#B07D2A", desc: "Underwhelming for the price or category. Better alternatives exist and are linked in the review." },
  { range: "1–4", label: "Poor / Avoid", color: "#C4622D", desc: "Significant formula flaws, deceptive labelling, prop blend abuse, or severe value failures." },
];

const conflicts = [
  { q: "Do brands pay for reviews?", a: "No. Reviews are initiated by our editorial team, not by brands. Brands are never notified before a review is published." },
  { q: "Do you accept free products?", a: "Occasionally we receive samples. If so, it is disclosed in the review header. Receiving a sample does not improve or worsen a score — it only affects our ability to assess mixability, taste, and packaging." },
  { q: "How do affiliate links work?", a: "We earn a small commission if you buy through our links. This funds the site. It does not — under any circumstance — change scores or rankings. A product we link to may still score 5/10 if the formula doesn't warrant more." },
  { q: "Can a brand request removal of a review?", a: "No. We do not remove reviews on commercial grounds. We will issue corrections if a formula changes, and we update scores when new evidence emerges." },
  { q: "Who writes the reviews?", a: "All editorial content is produced by the Fitlabreviews team. We do not accept guest posts from supplement brands, PR agencies, or paid contributors." },
];
...
export default function EditorialPolicyPage() {
  return (
    <div style={{ backgroundColor: "#F2EBD9" }}>
      ...
      <div style={{ borderBottom: "1px solid #D4C9B8", padding: "64px 24px 52px" }}>
        <div style={{ maxWidth: 860, margin: "0 auto" }}>
          ...
          <div style={{ marginTop: 32, display: "flex", flexWrap: "wrap", gap: 8 }}>
            {["Independence Guarantee", "Our Methodology", "Scoring Scale", "FSP System", "Conflict of Interest", "Corrections Policy"].map((item) => (
              <a key={item} href={`#${item.toLowerCase().replace(/\s+/g, "-")}`} style={{ padding: "5px 12px", border: "1px solid #D4C9B8", borderRadius: 20, fontSize: 11, color: "#5C5650", fontFamily: "var(--font-dm-mono), monospace", letterSpacing: "0.06em", textDecoration: "none", backgroundColor: "#F8F2E4" }}>
                {item}
              </a>
            ))}
          </div>
      ...
      <div style={{ maxWidth: 860, margin: "0 auto" }} className="pad-section-sm px-page">

        {/* Independence Guarantee */}
        <section id="independence-guarantee" style={{ marginBottom: 64, paddingBottom: 64, borderBottom: "1px solid #D4C9B8" }}>
          ...
          <div style={{ padding: "24px 28px", backgroundColor: "#1A1714", borderRadius: 12, marginBottom: 24 }}>
            <p style={{ fontFamily: "var(--font-playfair), Georgia, serif", fontSize: 17, fontStyle: "italic", color: "#F2EBD9", lineHeight: 1.65, margin: 0 }}>
        ...
        {/* Methodology */}
        <section id="our-methodology" style={{ marginBottom: 64, paddingBottom: 64, borderBottom: "1px solid #D4C9B8" }}>
          <h2 style={{ fontFamily: "var(--font-playfair), Georgia, serif", fontSize: "1.5rem", fontWeight: 700, color: "#1A1714", marginBottom: 24, letterSpacing: "-0.02em" }}>
            Our Methodology
          </h2>
          <div style={{ display: "flex", flexDirection: "column", gap: 0, border: "1px solid #D4C9B8", borderRadius: 12, overflow: "hidden" }}>
            {methodology.map((m, i) => (
              <div key={m.step} style={{ display: "grid", gridTemplateColumns: "44px 1fr", gap: 24, padding: "24px 24px", borderBottom: "1px solid #EDE8DF", backgroundColor: i % 2 === 0 ? "#F8F2E4" : "#F2EBD9" }}>
                <span style={{ fontFamily: "var(--font-dm-mono), monospace", fontSize: 12, color: "#C4622D", fontWeight: 700, paddingTop: 2 }}>{m.step}</span>
                <div>
                  <h3 style={{ fontFamily: "var(--font-playfair), Georgia, serif", fontSize: 15, fontWeight: 700, color: "#1A1714", marginBottom: 8 }}>{m.title}</h3>
        ...
        {/* Scoring Scale */}
        <section id="scoring-scale" style={{ marginBottom: 64, paddingBottom: 64, borderBottom: "1px solid #D4C9B8" }}>
          ...
          <p style={{ fontSize: 14, color: "#8A8480", fontFamily: "var(--font-dm-mono), monospace", marginBottom: 24 }}>Scores are assigned on a 1–10 integer scale only — no half-points.</p>
          <div style={{ display: "flex", flexDirection: "column", gap: 0, border: "1px solid #D4C9B8", borderRadius: 12, overflow: "hidden" }}>
            {scoring.map((s, i) => (
              <div key={s.range} style={{ display: "grid", gridTemplateColumns: "56px 1fr", gap: 20, padding: "18px 24px", borderBottom: "1px solid #EDE8DF", backgroundColor: i % 2 === 0 ? "#F8F2E4" : "#F2EBD9", alignItems: "start" }}>
                <div>
                  <span style={{ fontFamily: "var(--font-playfair), Georgia, serif", fontSize: 22, fontWeight: 800, color: s.color, display: "block", lineHeight: 1 }}>{s.range}</span>
                  <span style={{ fontFamily: "var(--font-dm-mono), monospace", fontSize: 9, color: "#8A8480", letterSpacing: "0.1em", textTransform: "uppercase" }}>{s.label}</span>
                </div>
                <p style={{ fontSize: 14, color: "#5C5650", lineHeight: 1.7, margin: 0, paddingTop: 2 }}>{s.desc}</p>
              </div>
            ))}
          </div>
        </section>
        ...
        {/* FSP System */}
        <section id="fsp-system" style={{ marginBottom: 64, paddingBottom: 64, borderBottom: "1px solid #D4C9B8" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(240px, 1fr))", gap: 12, marginBottom: 20 }}>
            {[
              ...
              <div key={p.label} style={{ padding: "16px 18px", border: "1px solid #D4C9B8", borderRadius: 8, backgroundColor: "#F8F2E4" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
                  <p style={{ fontFamily: "var(--font-playfair), Georgia, serif", fontSize: 13, fontWeight: 700, color: "#1A1714", margin: 0 }}>{p.label}</p>
                  <span style={{ fontFamily: "var(--font-dm-mono), monospace", fontSize: 10, color: "#C4622D", fontWeight: 700 }}>{p.weight}</span>
                </div>
                <p style={{ fontSize: 12, color: "#8A8480", margin: 0 }}>{p.desc}</p>
              </div>
            ))}
          </div>
          <div style={{ padding: "14px 18px", backgroundColor: "#EDE8DF", border: "1px solid #D4C9B8", borderRadius: 8, borderLeft: "3px solid #C4622D" }}>
            <p style={{ fontSize: 13, color: "#5C5650", lineHeight: 1.6, margin: 0 }}>
              ...
              <Link href="/methodology" style={{ color: "#C4622D", fontWeight: 600 }}>Read the full FSP methodology →</Link>
            </p>
          </div>
        </section>

        {/* Conflict of Interest */}
        <section id="conflict-of-interest" style={{ marginBottom: 64, paddingBottom: 64, borderBottom: "1px solid #D4C9B8" }}>
          <h2 style={{ fontFamily: "var(--font-playfair), Georgia, serif", fontSize: "1.5rem", fontWeight: 700, color: "#1A1714", marginBottom: 24, letterSpacing: "-0.02em" }}>
            Conflict of Interest — Q&A
          </h2>
          <div style={{ display: "flex", flexDirection: "column", gap: 0, border: "1px solid #D4C9B8", borderRadius: 12, overflow: "hidden" }}>
            {conflicts.map((c, i) => (
              <div key={i} style={{ padding: "20px 24px", borderBottom: "1px solid #EDE8DF", backgroundColor: i % 2 === 0 ? "#F8F2E4" : "#F2EBD9" }}>
                <p style={{ fontFamily: "var(--font-playfair), Georgia, serif", fontSize: 15, fontWeight: 700, color: "#1A1714", marginBottom: 8 }}>Q. {c.q}</p>
                <p style={{ fontSize: 14, color: "#5C5650", lineHeight: 1.7, margin: 0 }}>{c.a}</p>
              </div>
            ))}
          </div>
        </section>
        ...
        {/* Corrections Policy */}
        <section id="corrections-policy" style={{ marginBottom: 32 }}>
          <h2 style={{ fontFamily: "var(--font-playfair), Georgia, serif", fontSize: "1.5rem", fontWeight: 700, color: "#1A1714", marginBottom: 16, letterSpacing: "-0.02em" }}>
            Corrections Policy
          </h2>
          <p style={{ fontSize: 15, color: "#2D2926", lineHeight: 1.8, marginBottom: 16 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 24 }}>
            {[
              ...
              <div key={i} style={{ display: "flex", gap: 12, alignItems: "flex-start", padding: "12px 16px", backgroundColor: "#F8F2E4", border: "1px solid #D4C9B8", borderRadius: 8 }}>
                <span style={{ fontFamily: "var(--font-dm-mono), monospace", fontSize: 10, color: "#C4622D", fontWeight: 700, paddingTop: 2, flexShrink: 0 }}>0{i + 1}</span>
          ...
          <p style={{ fontSize: 14, color: "#5C5650", lineHeight: 1.7 }}>
            ...
            <a href="mailto:editorial@fitlabreviews.com" style={{ color: "#C4622D", fontWeight: 600 }}>editorial@fitlabreviews.com</a>.
