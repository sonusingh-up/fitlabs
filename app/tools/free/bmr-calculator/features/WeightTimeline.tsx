...
const S = {
  ...
  input: { width:"100%", padding:"10px 13px", borderRadius:8, border:"1px solid #E4E8E5", background:"#FDFAF4", fontSize:14, color:"#17211C", fontFamily:"var(--font-hanken),sans-serif", boxSizing:"border-box" as const },
  ...
  return (
    <div style={{ opacity:0, animation:`slideUp 250ms ${animDelay}ms ease-out forwards` }}>
      <div style={{ background:"#FDFAF4", border:"1px solid #E4E8E5", borderRadius:16, padding:"24px 24px 20px", marginBottom:16 }}>
        ...
        <h2 style={{ fontFamily:"var(--font-newsreader),serif", fontSize:20, fontWeight:700, color:"#17211C", marginBottom:16 }}>When will you reach your goal?</h2>
        ...
        <div className="timeline-inputs">
          <div>
            ...
            <input type="number" value={goalW} onChange={e => setGoalW(e.target.value)}
              ...
              style={{ ...S.input, borderColor: err.goalW ? "#EF4444" : "#E4E8E5" }} />
          ...
          <div>
            ...
            <input type="date" value={targetDate} min={minDate} max={maxDate}
              ...
              style={{ ...S.input, borderColor: err.date ? "#EF4444" : "#E4E8E5" }} />
        ...
        {result && (
          <div style={{ background:"#17211C", borderRadius:12, padding:"18px 20px" }}>
            ...
            {result.status === "ok" && (
              <p style={{ color:"#FFFFFF", fontSize:14 }}>At −500 kcal/day you'll reach your goal by <strong>{result.realisticDate}</strong>.</p>
            )}
            {result.status === "warn" && (
              <p style={{ color:"#FFFFFF", fontSize:14 }}>To hit your date you'd need to eat only <strong style={{ color:"#F59E0B" }}>{result.reqCals.toLocaleString()} kcal/day</strong> (−{result.reqDeficit} kcal deficit). This is aggressive but achievable with high protein intake.</p>
            )}
            {result.status === "danger" && (
              <>
                <p style={{ color:"#FFFFFF", fontSize:14, marginBottom:8 }}>That timeline requires a −{result.reqDeficit} kcal/day deficit — unsafe and likely to cause muscle loss.</p>
            ...
            <div className="timeline-milestones">
              {["Start", "4 wks", "8 wks", "12 wks"].map((label, i) => (
                <div key={label} style={{ background:"rgba(255,255,255,0.07)", borderRadius:8, padding:"8px 6px", textAlign:"center" }}>
                  <div style={{ fontSize:10, color:"#6B7770", marginBottom:3 }}>{label}</div>
                  <div style={{ fontSize:13, fontWeight:700, color:"#FFFFFF", fontVariantNumeric:"tabular-nums" }}>
