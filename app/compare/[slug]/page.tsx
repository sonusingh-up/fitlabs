    ...
    <div style={{ backgroundColor: "#FFFFFF" }}>
      <div style={{ borderBottom: "1px solid #E4E8E5", backgroundColor: "#F2F8F4" }}>
        <div style={{ maxWidth: 1280, margin: "0 auto", padding: "12px 24px", display: "flex", gap: 8, alignItems: "center" }}>
          ...
          <span style={{ fontSize: 12, color: "#3F4B43", fontFamily: "var(--font-jetbrains), monospace" }}>Compare</span>
        </div>
      </div>
      ...
      <div style={{ maxWidth: 1280, margin: "0 auto" }} className="pad-section">
        ...
        <div style={{ marginBottom: 56, textAlign: "center" }}>
          <p style={{ fontFamily: "var(--font-jetbrains), monospace", fontSize: 9, letterSpacing: "0.2em", textTransform: "uppercase", color: "#586259", marginBottom: 16 }}>DIRECT COMPARISON · 2026</p>
          <h1 style={{ fontFamily: "var(--font-newsreader), Georgia, serif", fontSize: "clamp(1.8rem, 4vw, 3rem)", fontWeight: 800, letterSpacing: "-0.025em", color: "#17211C", lineHeight: 1.05, marginBottom: 16 }}>
            {products[0].name} <em style={{ fontWeight: 400, fontStyle: "italic", color: "#3F4B43" }}>vs</em> {products[1].name}
          </h1>
          <p style={{ fontSize: 15, color: "#3F4B43", maxWidth: 540, margin: "0 auto" }}>
        ...
        <div className="layout-compare-cards" style={{ marginBottom: 56 }}>
          {products.map((p, i) => (
            <>
              <div key={p.slug} style={{ padding: 28, border: "1px solid #E4E8E5", borderRadius: 12, backgroundColor: "#F6F8F6" }}>
                <p style={{ fontFamily: "var(--font-jetbrains), monospace", fontSize: 10, letterSpacing: "0.12em", color: "#6B7770", marginBottom: 8 }}>{p.brand} · {p.type}</p>
                <h3 style={{ fontFamily: "var(--font-newsreader), Georgia, serif", fontSize: "1.2rem", fontWeight: 700, color: "#17211C", marginBottom: 12 }}>{p.name}</h3>
                <div style={{ display: "flex", marginBottom: 12 }}>
                  <ReviewScoreBadge rating={p.rating} size="md" />
                </div>
                <p style={{ fontSize: 13, color: "#3F4B43" }}>Best for: {p.bestFor}</p>
              </div>
              {i === 0 && (
                <div key="vs" className="compare-vs-hide" style={{ textAlign: "center" }}>
                  <span style={{ fontFamily: "var(--font-newsreader), Georgia, serif", fontSize: "2rem", fontWeight: 400, fontStyle: "italic", color: "#E4E8E5" }}>vs</span>
                </div>
              )}
            </>
          ))}
            ...
            <table style={{ width: "100%", borderCollapse: "collapse", fontFamily: "var(--font-hanken), sans-serif" }}>
              <thead>
                <tr style={{ borderBottom: "1px solid #E4E8E5", backgroundColor: "#F2F8F4" }}>
                  <th style={{ padding: "12px 20px", textAlign: "left", fontSize: 10, letterSpacing: "0.15em", textTransform: "uppercase", color: "#6B7770", fontFamily: "var(--font-jetbrains), monospace", fontWeight: 500 }}>Metric</th>
                  {products.map((p) => (
                    <th key={p.slug} style={{ padding: "12px 20px", textAlign: "center", fontFamily: "var(--font-newsreader), Georgia, serif", fontSize: 14, fontWeight: 700, color: "#17211C" }}>{p.name}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {[
                  ...
                  <tr key={row.label} style={{ borderBottom: "1px solid #F2F8F4", backgroundColor: i % 2 === 0 ? "#F6F8F6" : "#FFFFFF" }}>
                    <td style={{ padding: "12px 20px", fontSize: 12, color: "#3F4B43", fontFamily: "var(--font-jetbrains), monospace", letterSpacing: "0.1em", textTransform: "uppercase" }}>{row.label}</td>
                    {row.vals.map((val, j) => (
                      <td key={j} style={{ padding: "12px 20px", textAlign: "center", fontSize: 14, color: "#17211C", fontWeight: 500 }}>{val}</td>
                    ))}
                  </tr>
                ))}
              </tbody>
        ...
        <section style={{ marginBottom: 56 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 0, border: "1px solid #E4E8E5", borderRadius: 12, overflow: "hidden" }}>
            {winnerByCategory.map((item, i) => (
              <div key={item.category} className="layout-winners-row" style={{ borderBottom: "1px solid #F2F8F4", backgroundColor: i % 2 === 0 ? "#F6F8F6" : "#FFFFFF" }}>
                <div style={{ padding: "14px 20px", fontFamily: "var(--font-jetbrains), monospace", fontSize: 11, color: "#3F4B43", letterSpacing: "0.08em", textTransform: "uppercase" }}>{item.category}</div>
                <div className="winners-col-border" style={{ padding: "14px 20px", borderLeft: "1px solid #F2F8F4" }}>
                  <span style={{ padding: "3px 10px", backgroundColor: "#17211C", color: "#FFFFFF", fontSize: 11, fontWeight: 600, borderRadius: 8, fontFamily: "var(--font-hanken), sans-serif" }}>{item.winner}</span>
                </div>
                <div className="winners-col-border" style={{ padding: "14px 20px", fontSize: 13, color: "#3F4B43", borderLeft: "1px solid #F2F8F4" }}>{item.note}</div>
              </div>
            ))}
          </div>
        </section>
        ...
        <section>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(260px, 1fr))", gap: 16 }}>
            <div style={{ padding: 28, backgroundColor: "#17211C", borderRadius: 12 }}>
              <p style={{ fontFamily: "var(--font-jetbrains), monospace", fontSize: 9, letterSpacing: "0.2em", textTransform: "uppercase", color: "#3F4B43", marginBottom: 12 }}>Choose If...</p>
              <h3 style={{ fontFamily: "var(--font-newsreader), Georgia, serif", fontSize: "1.2rem", fontWeight: 700, color: "#FFFFFF", marginBottom: 12 }}>{products[0].name}</h3>
              <ul style={{ listStyle: "none", padding: 0, margin: 0 }}>
                {["You want a proven, trusted formula", "Budget matters — good value", "You don't have lactose issues", "You want more flavour options"].map((r) => (
                  <li key={r} style={{ fontSize: 13, color: "#6B7770", padding: "4px 0" }}>· {r}</li>
                ))}
              </ul>
            </div>
            <div style={{ padding: 28, backgroundColor: "#F6F8F6", border: "1px solid #E4E8E5", borderRadius: 12 }}>
              <p style={{ fontFamily: "var(--font-jetbrains), monospace", fontSize: 9, letterSpacing: "0.2em", textTransform: "uppercase", color: "#586259", marginBottom: 12 }}>Choose If...</p>
              <h3 style={{ fontFamily: "var(--font-newsreader), Georgia, serif", fontSize: "1.2rem", fontWeight: 700, color: "#17211C", marginBottom: 12 }}>{products[1].name}</h3>
              <ul style={{ listStyle: "none", padding: 0, margin: 0 }}>
                {["You're lactose intolerant", "You're in a cutting phase", "You want the cleanest isolate", "Absorption speed matters"].map((r) => (
                  <li key={r} style={{ fontSize: 13, color: "#3F4B43", padding: "4px 0" }}>· {r}</li>
                ))}
              </ul>
            </div>
          </div>
