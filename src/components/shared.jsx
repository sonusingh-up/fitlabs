...
export const FLLogo = ({ size = 28, color = "currentColor" }) => (
...
export const Molecule = ({ width = 520, height = 520, opacity = 1, accent = "var(--accent)" }) => (
...
export const SmallMolecule = ({ size = 80, accent = "var(--accent)" }) => (
...
export const Icon = ({ name, size = 16, stroke = "currentColor", strokeWidth = 1.5 }) => {
...
// Evidence badge
export const EvidenceBadge = ({ level = "strong", label }) => {
  const map = {
...
export const ScorePill = ({ score = 8.7 }) => (
...
export const Stat = ({ n, unit, label }) => (
...
export const ProductImage = ({ shape = "tub", color = "var(--accent)", w = 220, h = 220, label }) => {
  const inner = (() => {
    if (shape === "tub") return (
      <g>
...
export const Meter = ({ value = 0.7, color = "var(--accent)", label, valueLabel }) => (
  <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
    {label && (
...
// Citation/study row
export const StudyRow = ({ year, type, title, journal, n, link = true }) => (
