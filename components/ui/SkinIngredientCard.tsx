...
export interface SkinIngredientCardProps {
  slug: string;
  title: string;
  category?: string;
  description?: string;
...
const defaultStyle = evidenceStyle.insufficient;
...
export default function SkinIngredientCard({
  slug,
  title,
  category,
  description,
  benefits = [],
  evidenceLevel,
}: SkinIngredientCardProps) {
  const style = evidenceLevel ? (evidenceStyle[evidenceLevel] ?? defaultStyle) : defaultStyle;

  return (
    <Link
      href={`/skin/ingredients/${slug}`}
      style={{
        display: "block",
    ...
    >
      ...
      <div style={{ height: 110, background: style.bg, position: "relative", overflow: "hidden", padding: "16px 20px", display: "flex", flexDirection: "column", justifyContent: "space-between" }}>
        ...
        <div style={{ position: "relative", display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
          <span style={{ fontFamily: "var(--font-dm-mono), monospace", fontSize: 9, letterSpacing: "0.22em", textTransform: "uppercase", color: "rgba(242,235,217,0.3)" }}>Skin Ingredient</span>
          {category && (
            <span style={{ fontFamily: "var(--font-dm-mono), monospace", fontSize: 9, letterSpacing: "0.18em", textTransform: "uppercase", color: style.text }}>{category}</span>
          )}
        </div>
      ...
      <div style={{ padding: "18px 20px 22px" }}>
        ...
        {benefits.length > 0 && (
          <div style={{ padding: "10px 12px", backgroundColor: "#EDE8DF", borderRadius: 6, borderLeft: `3px solid ${style.accent}`, marginBottom: 16 }}>
            <p style={{ fontSize: 10, color: "#8A8480", fontFamily: "var(--font-dm-mono), monospace", letterSpacing: "0.12em", textTransform: "uppercase", marginBottom: 6 }}>Top Benefits</p>
            {benefits.slice(0, 2).map((benefit) => (
              <p key={benefit} style={{ fontSize: 12, color: "#2D2926", fontWeight: 500, lineHeight: 1.5, marginBottom: 2 }}>· {benefit}</p>
            ))}
          </div>
        )}
